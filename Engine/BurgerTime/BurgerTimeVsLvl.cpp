...
void BurgerTimeVsLvl::Notify(Component*, Event event)
{
	...
	if (event == Event::PLAYER_DIED)
	{
		...
		int HighScore{ BurgerTimeData::GetInstance().GetHeighScore(BurgerTimeData::GameMode::Vs) };
		int currScore{ m_pScoreCounter->GetComponent<ScoreCounterComp>()->GetScore() };

		if (HighScore < currScore)
			BurgerTimeData::GetInstance().SetHeighScore(BurgerTimeData::GameMode::Vs, currScore);
		BurgerTimeData::GetInstance().SetScore(BurgerTimeData::GameMode::Vs, currScore);
